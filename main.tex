...
\author{Jonathan~Dyer\thanks{Former Chief Engineer, Skybox Imaging, Terra Bella},~\IEEEmembership{Member,~IEEE,}
        Paul~Boerner\thanks{Imaging Systems Engineer, Planet, Inc.},~\IEEEmembership{Member,~IEEE}
        Dirk~Robinson\thanks{Former Imaging Systems Lead, Skybox Imaging, Terra Bella},~\IEEEmembership{Member,~IEEE,}
        and~Mark~Robertson\thanks{Former Lead Image Processing, Skybox Imaging, Terra Bella}%
}

\markboth{Journal of Remote Sensing .... BLAH BLAH}%
{}
...
We argue that a) system size reduction is key to realizing cost reduction and constellation scaling, b) commercial technology trends in imaging enable a level of imaging performance / cost heretofore impossible, c) new imaging architectures and technologies are required to fully realize these improvements and d) the exciting trends identified here have not yet been fully realized.
\end{abstract}
...
The second is that given other system constraints (detector line rate, $SNR$ required, focal plane dimension)
...
Happily, the trend towards capable commercial image detectors with smaller pixels opens up the opportunity of driving to towards higher $Q$'s where image quality is optimized without increasing system size.
...
\begin{equation}
\label{eq:I_D_phi}
I_D \propto \frac{\psi_{px}}{ACR \left(N_{e^-}^{FWC}\right)^3}
\end{equation}
...
The combination of high-frame-rate sensors and robust on-board processing offers the potential to dramatically increase the usable data that can be obtained from bandwidth-limited systems and enable full utilization of high $\phi_{px}$ sensors.
...
\begin{enumerate}
\item Leverages the high $\phi_{px}$ of newer sensors 
