...
\includefigure{figures/SNR_mtf_Q1.pdf}{fig:snr_mtf_q1}{Image demonstrating the impact of SNR and MTF (diffraction / blur) on resolvability for $Q=1$}

\includefigure{figures/SNR_mtf_Q2.pdf}{fig:snr_mtf_q2}{Image demonstrating the impact of SNR and MTF (diffraction / blur) on resolvability for $Q=2$}

More holistic image quality formulations typically refer back to \emph{resolve-ability} or the ability to distinguish objects or features of a given size in an image.  We will define a resolve-ability metric based on the NIIRS scale.
...
For most modern detectors, uncorrelated noise sources are small compared with well depth so we can approximate eq. \ref{eq:snr_alpha_multiframe}
...
and from this we can see that $I_D$ scales as

\begin{equation}
    \label{eq:I_D_scaling}
    I_D \sim \left[\frac{DR}{N_{e^-}^{FWC}}\right]^2 \textrm{log}_2 (DR)
\end{equation}

Equation \ref{eq:I_D_scaling} is important because it captures the inverse relationship of pixel full well capacity and data intensity.  Stated differently, in order to utilize smaller pixels with associated smaller full-well capacity (Figure \ref{fig:p_fwc}) the application must either tolerate less dynamic range in the output imagery or the system must oversample digitally and be capable of handling a higher data intensity.
...
\begin{equation}
\label{eq:lr_framing}    
LR = \left(y_{pix}FPS\right)_{det}
\end{equation}
...
Expanding eq. \ref{eq:A_fov}, it can be shown that, for a circular aperture, the illuminated focal plane radius is
...
Eq. \ref{eq:r_fp} clearly illustrates how important pixel pitch, $p_{px}$ is to minimizing the physical size of detector arrays and motivates the large push towards smaller pixels that has occured in commercial products.
...
We also showed in eq. \ref{eq:I_D_scaling} that data pixel FWC is very important to dynamic range and data intensity.
